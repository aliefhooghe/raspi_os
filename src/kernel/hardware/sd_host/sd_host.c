...
static void _sdhost_wait_for_cmd_done(void)
{
    ...
    for (;;) {
        ...
        if (SDCARD_INTERUPT_CMD_DONE & interupt) {
            kernel_log("sdcard: CMD was done");
        ...
        if (interupt & SDCARD_INTERUPT_CTO_ERR) {
            kernel_log("sdcard: timeout on commmand line");
        }
        if (interupt & SDCARD_INTERUPT_CTO_ERR) {
            kernel_log("sdcard: command crc error");
        }
        counter++;
        cpu_delay(150);
        if (!(counter & 0xFF)) {
...
static void _sdhost_send_data_command(
    ...
    sdcard_cmd_dir_t direction)
{
    kernel_log("sdcard: send CMD%u to sdcard", cmd);
...
{
    kernel_log("sdcard: initialize controller");

    // RESET
    const uint32_t mask = (1u << 24);
    mmio_write(REG__SDCARD_CONTROL1, mask);
    while ((mmio_read(REG__SDCARD_CONTROL1) & mask)) {
        kernel_log("sdcard: wait for device (RESET)");
        asm("nop");
    }
    kernel_log("sdcard: RESET DONE:");
    ...
    kernel_log("sdcard: negotiation is done.");
    ...
    for (;;) {
        ...
        // here: 
        kernel_log("sdcard: response for ACMD41 is 0x%x", operation_condition );

        if (operation_condition & SDCARD_ACMD41_RESP_POWER_UP)
        {
            kernel_log("sdcard: card is powered up.");

            if (operation_condition & SDCARD_ACMD41_RESP_HIGH_CAPACITY)
            {
                kernel_log("sdcard: card type: SDHC/SDXC");
            }
            else
            {
    ...
    for (int i = 0; i < 4; i++) {
        kernel_log("sdcard: CID: RESP%d = 0x%x", i, resps[i]);
    }
    ...
    kernel_log("sdcard: Relative Card Address = 0x%x", relative_card_address);
    kernel_log("sdcard: card identification is done");
    ...
    kernel_log("sdcard: sdcard status is %x", state);

    if (state == SDCARD_STATUS_CURRENT_STATE_TRANSFERT)
    {
        kernel_log("sdcard: card is ready for transfert");
    }
    ...
    kernel_log("sdcard: sd host driver is initialized");
}

int sdhost_read_block(uint32_t block_index, uint8_t block[512])
{
    kernel_log("sdcard: read block @ %u", block_index);
    ...
    for (;;) {
        ...
        {
            kernel_log("sdcard: data is ready");
            break;
        }
